...
// ─── Custom Bar Chart Tooltip (outside component) ────────────
...
export function TimeTracker() {
  ...
  return (
    <div className="space-y-6">
      {/* ─── Active Timer Card ──────────────────────── */}
      <Card className={`border-2 ${activeTimer && !isPaused ? 'border-emerald-500/60' : 'border-zinc-800'} bg-zinc-900/60`}>
                  ...
                  >
                    ...
                    <SelectContent className="bg-zinc-900 border-zinc-800">
                      {tasks
                        ...
                        .map(t => (
                          <SelectItem key={t.id} value={t.id} className="text-zinc-300">
                            {t.title}
                          </SelectItem>
                        ))}
                    </SelectContent>
                  ...
                  {!activeTimer ? (
                    ...
                    <>
                      ...
                      <Button
                        ...
                        size="sm"
                        onClick={handleStop}
                        className="h-9 border-red-600 text-red-400 hover:bg-red-600/10"
                      >
                ...
                <div className="space-y-4 py-2">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1.5">
                      ...
                      <Select
                        ...
                        onValueChange={v => setManualEntry(prev => ({ ...prev, taskId: v }))}
                      >
                    ...
                    <div className="space-y-1.5">
                      ...
                      <Select
                        ...
                        onValueChange={v => setManualEntry(prev => ({ ...prev, projectId: v }))}
                      >
                        <SelectTrigger className="bg-zinc-900 border-zinc-800 text-zinc-300 text-sm">
                  ...
                  <div className="flex items-center gap-2">
                    <Switch
                      ...
                      onCheckedChange={v => setManualEntry(prev => ({ ...prev, isBillable: v }))}
                    />
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-2 mt-0.5">
                            {task && (
                              <span className="text-[10px] text-zinc-500">{task.title}</span>
                            )}
                            {project && (
        ...
        <Card className="bg-zinc-900/60 border-zinc-800">
          ...
          <CardContent>
            <div className="space-y-4">
                  ...
                  <div key={goal.id} className="space-y-2">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        {goal.completed && (
                          <Badge variant="outline" className="text-[9px] h-4 px-1 border-emerald-600/40 text-emerald-400">
                            Atteint
                          </Badge>
                        )}
                      </div>
                      <span className="text-sm text-zinc-400">
                        {isHours
                          ? `${current.toFixed(1)}h / ${target.toFixed(1)}h`
              ...
              {/* Encouraging message */}
              {(() => {
                const hoursGoal = billingGoals.find(g => g.unit === 'hours')
                if (hoursGoal && hoursGoal.target) {
                  const remaining = Math.max((hoursGoal.target || 0) - (hoursGoal.current || 0), 0)
                  return (
                    <p className="text-xs text-zinc-500 pt-2 border-t border-zinc-800">
                      {remaining > 0
                        ? `Il vous reste ${remaining.toFixed(1)} heures facturables cette semaine`
                        : 'Bravo ! Vous avez atteint votre objectif d\'heures facturables ! 🎉'
                      }
                    </p>
                  )
                }
