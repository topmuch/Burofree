...
function formatPdf(
  ...
  const title = `Rapport ${entityType === 'all' ? 'complet' : entityType} — Burozen`
...
<div class="footer">Burozen — Assistant Intelligent Freelance</div>
</body></html>`
...
function generateFileName(options: ExportOptions): string {
  ...
  return `burozen_${options.entityType}_${date}`
}
